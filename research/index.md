...
.overlay-content{
  ...
  overflow:auto; /* 内容过长时图片内滚动，绝不溢出 */
  line-height:1.4;font-size:.95rem;
  text-wrap:balance; /* 支持的浏览器里可减少“丑”换行 */
  word-break:normal;overflow-wrap:break-word
}
.overlay-content p{margin:0 0 8px}
.overlay-title{margin:0 0 6px;font-weight:700}
.close-overlay{
  position:absolute;top:8px;right:8px;background:#0000;border:0;
  border-radius:6px;padding:6px 10px;cursor:pointer;color:#111;font-weight:700
}
...
<!-- topic 文本：用 <template>，不参与 JS 解析，避免奇怪转义 -->
...
(function(){
  function openOverlay(btn){
    ...
    if (tpl && !contentBox.getAttribute('data-loaded')){
      contentBox.innerHTML = tpl.innerHTML;     // 注入模板 HTML
  ...
  function closeOverlay(overlay){
    ...
    var controlBtn = document.querySelector('[aria-controls="'+ overlay.id +'"]');
  ...
  for (var i=0;i<buttons.length;i++){
    buttons[i].addEventListener('click', function(){ openOverlay(this); });
  }
  ...
  for (var j=0;j<closers.length;j++){
    closers[j].addEventListener('click', function(e){
      closeOverlay(e.currentTarget.closest('.research-overlay'));
    });
  }

  // 可选：点击遮罩空白处也关闭
  document.addEventListener('click', function(e){
    var ov = e.target.closest('.research-overlay');
